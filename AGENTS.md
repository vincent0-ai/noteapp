...
**Date:** 2026-06-06
**Changes:**
- **Shared Links tab — first-character avatar + lock badge (`HomeScreen.kt`)** — Each `SharedNoteCard` now has a Gmail-style circular avatar showing the first non-whitespace character of the note's content (after `stripMarkdown`). The accent colour is derived from the character's codepoint so the same note always renders the same colour across reloads. When `note.isLocked == true`, a small red lock badge sits in the bottom-right corner of the avatar (with a thin surface-coloured border so it stays readable on any background) and a "Locked on website" subtitle appears next to the first-line preview. The Manage button stays in the right corner.
- **Shared Links — synthetic-note fallback now respects server lock state** — `NotesViewModel.loadActiveShares` was building the synthesised `AppNote` (used when the local cache doesn't have the note) with a hard-coded `isLocked = false`, so the lock badge would disappear on devices that hadn't synced the underlying note yet. It now reads `dto.is_locked` from the (newly extended) `ActiveShareDto` so the lock badge stays correct.
- **`ActiveShareDto` gains `is_locked: Boolean = false`** — wire-compatible: existing client deserializers that don't know the field will fall through to the default, so the field is safe to add in a single cut. The fake service and the main service both default to false.
- **Hide locked notes from search (mirror the web platform's behaviour)** — The web platform never shows locked notes in search results until the user unlocks them, and the Android app now does the same. Three layers of defence:
  1. **Server already filters** — `/personal_post/search` in `blueprints/notes.py` already excludes `is_locked: True` from both the Typesense and the MongoDB fallback paths. No server change needed.
  2. **Offline-search fallback now filters too** — `NotesRepository.searchNotes` was returning all local notes from `dbHelper.getAllNotes()` when the network was unavailable, which leaked locked notes offline. The line now reads `dbHelper.getAllNotes().filter { !it.isLocked }` before the query filter, so the offline fallback also mirrors the web platform.
  3. **SearchScreen filters the rendered list as a belt-and-braces defence** — A `notePairs` list is built once via `remember(results)` and `visiblePairs` filters out any hit whose `note?.isLocked == true` (defends against partial sync, older server, etc.). The empty-state copy now distinguishes "no results" from "locked notes were hidden — unlock them in the Locked tab".
- **Version bump** — `versionCode 9 → 10`, `versionName 1.7.1 → 1.7.2`. New APK pushed to `static/downloads/app-debug.apk` and `static/update-manifest.json` updated.
**Files touched:** `app/src/main/java/com/example/echowithin/data/model/ApiModels.kt` (added `is_locked` to `ActiveShareDto`), `app/src/main/java/com/example/echowithin/presentation/viewmodel/NotesViewModel.kt` (synthetic-note fallback reads `dto.is_locked`), `app/src/main/java/com/example/echowithin/presentation/screens/HomeScreen.kt` (new `FirstCharAvatar` composable, restructured `SharedNoteCard` header, imports for `sp` / `clip` / `border`), `app/src/main/java/com/example/echowithin/data/repository/NotesRepository.kt` (offline search filters locked notes), `app/src/main/java/com/example/echowithin/presentation/screens/SearchScreen.kt` (filters locked notes from results, smarter empty-state copy), `app/build.gradle.kts` (version bump).
**Verification:** `./gradlew.bat compileDebugKotlin` and `./gradlew.bat assembleDebug` both pass. Only pre-existing deprecation warnings remain.
**Privacy Note:** No new data exposure. The avatar is built from the *first character* of locally-decrypted content; the content itself is never sent to the server. The new `is_locked` field on the shares response is read from the same `personal_posts_conf` document the endpoint already returns the title from, scoped to `ObjectId(current_user.id)`. The search filter is a read-time operation — it never transmits a different request to the server than it did before.

### Model: opencode/minimax-m3-free
**Date:** 2026-06-06
**Changes:**
- **SECURITY: Removing PIN protection now requires the current PIN** — The "Remove PIN Protection" button in the AppLock settings screen used to call the server with zero credentials, and the corresponding `/api/v1/app_lock/remove` endpoint accepted a bare POST and unset `app_lock_pin_hash` without re-verifying. The result: anyone with the app open (a borrowed device, a thief past the OS lockscreen) could disable the app-lock without knowing the PIN. The screen now requires the user to type the current 4-digit PIN in a dedicated field before the button enables, the PIN is sent in the request body, the server re-hashes it and runs `check_password_hash` against the stored bcrypt hash, and only then unsets it. Wrong PIN shakes the field red (existing `LaunchedEffect(error)` shake animation) and shows the server's `"Incorrect PIN."` text. Stale errors clear the moment the user starts typing a new PIN (new `onClearError` callback wired to `AppLockViewModel.clearError()`).
- **"Forgot PIN? Reset on the website" link** — A new underlined text link sits below the action buttons in AppLock settings, visible in both the "active" and "locked" states. Tapping it opens `https://echowithin.xyz/profile/<username>/settings` (or the bare `https://echowithin.xyz` if the cached username is blank) in the system browser via `Intent.ACTION_VIEW`. The website already has the full email-based reset flow (6-digit code → new PIN) on that page, so we don't have to re-implement the wizard inside the app. Uses the existing `BrandOrange` colour and `TextDecoration.Underline` so the affordance reads as a link without introducing a new colour or icon.
- **BUG: Mark-all-read no longer 400s** — The Activity tab's "Mark all as read" button was silently failing. The four endpoints it touches — `GET api/notifications/badge-counts`, `GET api/posts/my-commented`, `POST api/posts/mark-all-read`, `POST api/activity/mark_read` — exist on the server in two flavors each. The `/api/*` (legacy, no `v1`) versions live in the blog / chat / notes blueprints and are NOT CSRF-exempt (`csrf.exempt` is only called on `api_bp` in `main.py:140`), so the Android POSTs were rejected by the CSRF middleware with HTTP 400 and the optimistic UI was rolled back by the next `loadNotifications()` round-trip. Switched all four Retrofit annotations to the v1 equivalents which live under `api_bp` and are fully CSRF-exempt. The switch also brings the GETs and POSTs onto the same data model (`activity_read_conf`), so the unread badge, the notifications feed, and the mark-all-read button all read/write the same rows.
- **PREFERENCE: syncMode actually respected on reconnect** — The sync preference ("Automatic" / "Manual" in Settings) was only honored at app launch (`loadAllData` gated on `syncMode == "automatic"`) and on the manual Sync button. The reconnect path — `onConnectivityChanged()` pings a `_syncTrigger`, which a `LaunchedEffect(syncTrigger)` in `AppNavGraph` consumes to call `syncNotes()` — ignored the preference entirely, so "Manual Sync" users were still being auto-pushed the moment connectivity flipped to online. Fix: `onConnectivityChanged()` now early-exits if `syncMode != "automatic"` (it still updates `pendingSyncCount` so the offline banner can show "N changes pending" — the pending state is real, the user just has to tap Sync to push them). The `LaunchedEffect` also gates on `syncMode` as defense in depth, so any future code path that pings the trigger will be safe by default.
- **CLEANUP: deleted `FakeEchoWithinApiService.kt` entirely (244 lines)** — The in-memory fake from early development is gone. The stale "swap to FakeEchoWithinApiService" comment in `ApiClient.kt` is gone too. Production has always used the real retrofit client via `retrofit.create(EchoWithinApiService::class.java)`, so this is a pure deletion with no behaviour change. The only remaining `FakeEcho` references are in committed history (AGENTS.md session log entries from previous versions), which are intentionally left intact.
- **Version bump** — `versionCode 10 → 11`, `versionName 1.7.2 → 1.7.3`. New APK pushed to `static/downloads/app-debug.apk` (21,616,179 bytes) and `static/update-manifest.json` updated with the new changelog.
**Files touched:** `app/build.gradle.kts` (version bump), `data/model/ApiModels.kt` (added `AppLockRemoveDto(pin)`), `data/network/ApiClient.kt` (removed stale comment), `data/network/EchoWithinApiService.kt` (added `removeAppLock(@Body AppLockRemoveDto)`, all four notif/badge endpoints moved to `api/v1/...`), `data/network/FakeEchoWithinApiService.kt` (DELETED), `data/repository/AppLockRepository.kt` (`removeLock(pin)` with offline-blocked behaviour preserved, sends the PIN in the body), `presentation/screens/AppLockScreen.kt` (new PIN field for the "active / about-to-remove" state, "Forgot PIN?" deep-link, `onClearError` callback, `LocalContext` import, `clickable` import, `TextDecoration` import), `presentation/viewmodel/AppLockViewModel.kt` (`remove(pin: String)` signature, `clearError()`), `presentation/viewmodel/NotesViewModel.kt` (`onConnectivityChanged` early-exits when `syncMode != "automatic"`), `presentation/navigation/AppNavGraph.kt` (`LaunchedEffect(syncTrigger)` also gates on `syncMode`; `onRemove` passes the PIN through).
**Verification:** `./gradlew.bat assembleDebug` — BUILD SUCCESSFUL in 13s. APK is 21,616,179 bytes. Pushed to `static/downloads/app-debug.apk`. `static/update-manifest.json` updated to `versionCode 11 / 1.7.3`.
**Privacy Note:** The PIN-gated removal closes a real bypass: a thief with the unlocked device could previously disable the app-lock without knowing the PIN. The new flow requires possession of the current PIN; the PIN itself is never persisted on the server (only its bcrypt hash) and the client only sends it in the request body for the duration of the verify. The "Forgot PIN?" link is a deep-link to the user's own profile page on the website, no new PII is shared with the app. The mark-all-read fix changes the URL the client calls but adds no new fields, no new scopes, and uses the same `ObjectId(current_user.id)` server-side filter as before.
